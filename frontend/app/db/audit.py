"""
Ligant.ai Audit Logging Utilities
...
def log_action(
    ...
    The caller is responsible for committing the session (or relying on the
    ``get_db()`` context manager to auto-commit).

    Returns the newly created ``AuditLog`` instance.
    """
    entry = AuditLog(
        user_id=user_id,
...
def log_job_submitted(
    ...
    """Record that a user submitted a new RFdiffusion job."""
    return log_action(
        db,
        user_id=user_id,
        action_type="job.submitted",
...
def log_job_completed(
    ...
    """Record that a job completed (success or failure)."""
    return log_action(
        db,
        user_id=user_id,
...
def log_pdb_uploaded(
    ...
    """Record that a user uploaded a PDB file."""
    return log_action(
        db,
        user_id=user_id,
...
def log_chat_message(
    ...
    """Record that a chat message was created (user, assistant, or system)."""
    return log_action(
        db,
        user_id=user_id,
...
def log_viz_viewed(
    ...
    """Record that a user viewed a 3-D structure visualization."""
    return log_action(
        db,
        user_id=user_id,
        action_type="viz.structure_viewed",
        resource_type="pdb_file",
        resource_id=str(file_id),
    )
